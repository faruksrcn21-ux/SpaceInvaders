...
void SoundManager::sineWave(std::vector<sf::Int16> &samples,
                            unsigned sampleRate, float durationSec,
  ...
  for (int i = 0; i < count; i++) {
    ...
    float env =
        std::min(t / 0.005f, 1.f) * std::min((durationSec - t) / 0.01f, 1.f);
    samples[i] = static_cast<sf::Int16>(std::sin(2.f * PI * freq * t) *
...
void SoundManager::noiseWave(std::vector<sf::Int16> &samples,
                             unsigned sampleRate, float durationSec,
                             float amplitude, float decayRate) {
  int count = static_cast<int>(sampleRate * durationSec);
  samples.resize(count);
  for (int i = 0; i < count; i++) {
    float t = static_cast<float>(i) / sampleRate;
...
void SoundManager::buildUfoBuffer() {
  ...
  for (int i = 0; i < count; i++) {
    ...
    float currentFreq =
        carrierFreq + std::sin(2.f * PI * modFreq * t) * modAmount;
...
// 4 notalık klasik Space Invaders benzeri gerilim ritmi (Daha tok ve bas
// ağırlıklı)
void SoundManager::buildFleetBuffers() {
  ...
  for (int j = 0; j < 4; j++) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      // Frekans kayması (pitch slide): Vuruş hissini artırmak için frekansı
      // hafifçe düşürüyoruz
      ...
      s[i] = static_cast<sf::Int16>(val * envelope * 0.45f *
                                    32767.f); // Ses seviyesi dengelendi
    }
...
void SoundManager::buildMusicBuffer() {
  ...
  for (int n = 0; n < NOTE_COUNT; n++) {
    ...
    sineWave(noteSamples, SAMPLE_RATE, NOTE_DUR, notes[n], notes[n] * 0.98f,
             0.18f);
    allSamples.insert(allSamples.end(), noteSamples.begin(), noteSamples.end());
  }

  musicBuf_.loadFromSamples(allSamples.data(), allSamples.size(), 1,
                            SAMPLE_RATE);
}

// Powerup Toplama: 350Hz'den 1400Hz'e hızlıca sweep eden retro sinyal, 0.28 sn
void SoundManager::buildPowerupPickupBuffer() {
  std::vector<sf::Int16> s;
  sineWave(s, SAMPLE_RATE, 0.28f, 350.f, 1400.f, 0.45f);
  powerupPickupBuf_.loadFromSamples(s.data(), s.size(), 1, SAMPLE_RATE);
}

// Bomb Patlama: Çok kalın, tok baslı retro patlama (düşük decay gürültüsü), 1.0
// sn
void SoundManager::buildBombExplosionBuffer() {
  std::vector<sf::Int16> s;
  noiseWave(s, SAMPLE_RATE, 1.0f, 0.95f,
            3.2f); // yavaş sönümlenen kalın gürültü
  bombExplosionBuf_.loadFromSamples(s.data(), s.size(), 1, SAMPLE_RATE);
}
...
SoundManager::SoundManager() {
  ...
  buildFleetBuffers();
  buildMusicBuffer();
  buildPowerupPickupBuffer();
  buildBombExplosionBuffer();
  ...
  powerupPickupSnd_.setBuffer(powerupPickupBuf_);
  bombExplosionSnd_.setBuffer(bombExplosionBuf_);
  ...
  powerupPickupSnd_.setVolume(80.f);
  bombExplosionSnd_.setVolume(95.f);
}
...
void SoundManager::stopUfo() { ufoSnd_.stop(); }
...
void SoundManager::stopMusic() { musicSnd_.stop(); }
...
void SoundManager::toggleMute() {
  ...
  if (muted_) {
    musicSnd_.pause();
    ufoSnd_.pause();
    powerupPickupSnd_.pause();
    bombExplosionSnd_.pause();
  } else {
    musicSnd_.play();
...
bool SoundManager::isMuted() const { return muted_; }

void SoundManager::playPowerupPickup() {
  if (muted_)
    return;
  powerupPickupSnd_.stop();
  powerupPickupSnd_.play();
}

void SoundManager::playBombExplosion() {
  if (muted_)
    return;
  bombExplosionSnd_.stop();
  bombExplosionSnd_.play();
}