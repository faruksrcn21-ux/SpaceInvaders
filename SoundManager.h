...
private:
  ...
  void buildPowerupPickupBuffer();
  void buildBombExplosionBuffer();
  ...
  sf::SoundBuffer powerupPickupBuf_;
  sf::SoundBuffer bombExplosionBuf_;
  ...
  sf::Sound powerupPickupSnd_;
  sf::Sound bombExplosionSnd_;
  int fleetStep_ = 0;
  ...
  static void sineWave(std::vector<sf::Int16> &samples, unsigned sampleRate,
                       float durationSec, float startFreq,
                       float endFreq, // frekans sweep için
                       float amplitude);

  static void noiseWave(std::vector<sf::Int16> &samples, unsigned sampleRate,
